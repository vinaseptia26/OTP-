...
class _ApprovalLemburScreenState extends State<ApprovalLemburScreen>
  ...
  String? _fungsiFilterSuperadmin;
  ...
  // ============================================================================
  // BULK SELECTION (HANYA SUPERADMIN)
  // ============================================================================
    ...
    try {
      if (isSuperadmin) {
        final stats = await _approvalService.getStatisticsForSuperadmin(
          fungsiFilter: _fungsiFilterSuperadmin,
        );
        if (mounted) {
          setState(() {
        ...
        final stats = await _approvalService.getStatisticsForManager(_userFungsi!);
        if (mounted) {
          setState(() {
            _totalPending = stats['totalPending'] ?? 0;
            _totalApproved = stats['totalApproved'] ?? 0;
  ...
  Widget build(BuildContext context) {
    final title = isSuperadmin ? 'Approval Lembur (Admin)' : 'Approval Lembur';
    return Scaffold(
      ...
      body: _isLoading
          ...
          : Column(
              children: [
                ...
                _buildTabBar(),
                if (isSuperadmin && _isBulkMode && _tabController.index == 0 && _selectedIds.isNotEmpty)
                  _buildBulkActionBar(),
                Expanded(
  ...
  PreferredSizeWidget _buildAppBar(String title) {
    return AppBar(
      title: Text(title, style: GoogleFonts.poppins(fontWeight: FontWeight.w700, color: Colors.white, fontSize: 18)),
      ...
      actions: [
        ...
        if (isSuperadmin && _tabController.index == 0)
          PopupMenuButton<String>(
            icon: Icon(_fungsiFilterSuperadmin != null ? Icons.filter_alt : Icons.filter_list, color: _fungsiFilterSuperadmin != null ? Colors.orange : Colors.white),
            onSelected: (value) {
              setState(() {
                _fungsiFilterSuperadmin = value == 'semua' ? null : value;
                _selectedIds.clear();
                _isSelectAll = false;
              });
              _loadStatistics();
        ...
        if (isSuperadmin && _totalPending > 0)
          IconButton(
      ...
      bottom: PreferredSize(
        ...
        child: Padding(
          ...
          child: Row(
            children: [
              Icon(isSuperadmin ? Icons.admin_panel_settings : Icons.person, color: Colors.white.withOpacity(0.8), size: 20),
              const SizedBox(width: 8),
              Expanded(
                child: Text(
                  '${isSuperadmin ? 'Superadmin' : 'Manager'}: ${_userName ?? "Loading..."}',
                  style: GoogleFonts.poppins(color: Colors.white.withOpacity(0.9), fontSize: 12, fontWeight: FontWeight.w500),
                  maxLines: 1,
                  overflow: TextOverflow.ellipsis,
                ),
              ),
              if (isManager && _userFungsi != null) ...[
                const SizedBox(width: 8),
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                  decoration: BoxDecoration(
                    color: _getFungsiColor(_userFungsi).withOpacity(0.3),
                    borderRadius: BorderRadius.circular(12),
                    border: Border.all(color: _getFungsiColor(_userFungsi).withOpacity(0.5)),
                  ),
                  child: Text(_getFungsiLabel(_userFungsi).toUpperCase(), style: GoogleFonts.poppins(color: Colors.white, fontSize: 10, fontWeight: FontWeight.w700)),
                ),
              ],
            ],
          ),
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          if (isSuperadmin && _perFungsi.isNotEmpty) ...[
          ...
          if (isManager && _userFungsi != null) ...[
            ...
            Container(
              ...
              child: Row(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Icon(Icons.business, color: _getFungsiColor(_userFungsi), size: 16),
                  const SizedBox(width: 8),
                  Text('Fungsi: ${_getFungsiLabel(_userFungsi)}', style: GoogleFonts.poppins(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w500)),
                ],
              ),
            ),
          ],
  ...
  Widget _buildPendingTab() {
    return StreamBuilder<List<Map<String, dynamic>>>(
      stream: isSuperadmin
          ? _approvalService.getApprovalListForSuperadmin(status: 'pending', fungsiFilter: _fungsiFilterSuperadmin)
          : _approvalService.getApprovalListForManager(status: 'pending', fungsiManager: _userFungsi ?? ''),
  ...
  // ============== APPROVAL CARD (DENGAN NAMA MITRA) ==============
  ...
  Widget _buildFungsiDropdown() {
    return Container(
      ...
      child: DropdownButton<String>(
        value: _fungsiFilterSuperadmin ?? 'semua',
