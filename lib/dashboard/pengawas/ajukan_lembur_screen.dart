...
class _AjukanLemburPageState extends State<AjukanLemburPage>
  ...
  Widget _buildProgressIndicator() {
    ...
    if (tanggalLembur != null && jamMulai != null && jamSelesai != null)
      currentStep = 2;
  ...
  Widget _buildMitraSection() {
    return _buildCard(
      child: Column(
        ...
        children: [
          _buildSectionHeader(
              Icons.people_alt_rounded, "Pilih Mitra", const Color(0xFF0D47A1)),
            ...
            Container(
              ...
              child: ListView.separated(
                ...
                itemBuilder: (context, index) =>
                    _buildMitraItem(selectedMiras[index]),
              ),
            ),
            const SizedBox(height: 12),
          ],
          SizedBox(
            ...
            child: OutlinedButton.icon(
              ...
              onPressed:
                  fungsiPengawas != null ? _showMitraSelectionDialog : null,
            ),
          ),
        ],
      ),
        ...
        children: [
          ...
          InkWell(
            ...
            child: Container(
              ...
              child:
                  const Icon(Icons.close, size: 18, color: Color(0xFFD32F2F)),
            ),
          ),
        ],
      ),
  ...
  Future<void> _showMitraSelectionDialog() async {
    final result = await showDialog<List<Map<String, dynamic>>>(
      ...
      builder: (context) => MitraSelectionDialog(
          fungsi: fungsiPengawas!, selectedIds: selectedMitraIds),
    );
  ...
  Widget _buildWaktuSection() {
    return _buildCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _buildSectionHeader(Icons.access_time_filled, "Waktu Lembur",
          ...
          if (tanggalLembur != null) ...[
            ...
            Row(
              children: [
                ...
                const SizedBox(width: 12),
                Expanded(
                    child: _buildTimePicker("Jam Selesai", jamSelesai, false)),
              ],
            ),
          ],
            ...
            if (tanggalLembur != null)
              InkWell(
                ...
                child: Container(
                    ...
                    decoration: const BoxDecoration(
                        shape: BoxShape.circle, color: Color(0xFFFEEBEE)),
  ...
  Future<void> _pilihWaktu(bool isJamMulai) async {
    ...
    final selectedTime = await showTimePicker(
      ...
      initialTime: isJamMulai
          ? (jamMulai ?? const TimeOfDay(hour: 7, minute: 0))
          : (jamSelesai ?? const TimeOfDay(hour: 17, minute: 0)),
    );
  ...
  Widget _buildTimeSummary() {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(
              color: const Color(0xFF2E7D32).withOpacity(0.15), blurRadius: 8)
        ],
      ),
      child: Column(
        children: [
          _buildSummaryRow(
              "Durasi Lembur", "${totalJam.toStringAsFixed(1)} jam"),
          if (selectedMiras.isNotEmpty && _overtimeRates != null) ...[
            ...
            _buildSummaryRow(
                "Biaya per Mitra",
                _overtimeService.formatRupiah(selectedMiras.isNotEmpty
                    ? totalBiaya / selectedMiras.length
                    : 0)),
            if (selectedMiras.length > 1) ...[
              const SizedBox(height: 6),
              _buildSummaryRow(
                  "Total Biaya", _overtimeService.formatRupiah(totalBiaya),
  ...
  Widget _buildLokasiSection() {
    return _buildCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _buildSectionHeader(Icons.location_on_rounded, "Lokasi Lembur",
          ...
          Row(
            children: [
              Expanded(
                ...
                setState(() {
                  ...
                  _selectedLocation = LatLng(
                      LocationService.kantorLat, LocationService.kantorLng);
              ...
              const SizedBox(width: 12),
              Expanded(
                  child: _buildLocationTypeCard(Icons.construction_rounded,
                      "Proyek", "Luar Kantor", _locationType == 'proyek', () {
                setState(() {
          ...
          if (_locationType == 'proyek') ...[
            ...
            SizedBox(
              ...
              child: ElevatedButton.icon(
                ...
                label: const Text("Pilih Lokasi dari Peta Satelit",
                ...
                children: [
                  Expanded(
                      ...
                      child: TextFormField(
                          ...
                          decoration: _buildInputDecoration("RT", "001"))),
                  const SizedBox(width: 12),
                  Expanded(
                      flex: 2,
                      child: TextFormField(
                          controller: _rwController,
                          decoration: _buildInputDecoration("RW", "001"))),
                ],
              ),
              const SizedBox(height: 6),
              Text("Tambahkan RT/RW agar alamat lebih spesifik",
  ...
  Widget _buildLocationTypeCard(IconData icon, String title, String subtitle,
      ...
      child: Row(
        children: [
          Icon(
              ...
              color: _isOutsideRadius ? Colors.orange : Colors.green,
              size: 22),
          const SizedBox(width: 10),
          Expanded(
              child: Text(_getLokasiInfo(),
                  style: TextStyle(
                      fontSize: 13,
                      fontWeight: FontWeight.w500,
  ...
  String _getLokasiInfo() {
    ...
    if (_locationType == 'proyek') {
      if (_rtController.text.isNotEmpty)
        alamatLengkap += " RT ${_rtController.text}";
      if (_rwController.text.isNotEmpty)
        alamatLengkap += " RW ${_rwController.text}";
    }
    if (isOutside) {
  ...
  Future<void> _showMapPicker() async {
    ...
    final initialLng =
        _selectedLocation?.longitude ?? LocationService.kantorLng;
    final result = await showDialog<Map<String, dynamic>>(
      context: context,
      barrierDismissible: false,
      builder: (context) =>
          _MapPickerDialog(initialLat: initialLat, initialLng: initialLng),
    );
    if (result != null && mounted) {
      setState(() {
        _selectedLocation = LatLng(
  ...
  Widget _buildAlasanSection() {
    return _buildCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _buildSectionHeader(Icons.description_rounded, "Alasan Lembur",
              const Color(0xFF0D47A1)),
          const SizedBox(height: 12),
          TextFormField(
  ...
  Widget _buildUrgensiSection() {
    return _buildCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _buildSectionHeader(Icons.priority_high_rounded, "Tingkat Urgensi",
              const Color(0xFFE65100)),
          const SizedBox(height: 12),
          Row(
            children: [
              _buildUrgensiChip(
                  "Normal", "normal", Icons.info_outline_rounded, Colors.blue),
              const SizedBox(width: 10),
              _buildUrgensiChip("Tinggi", "tinggi", Icons.warning_amber_rounded,
                  Colors.orange),
              const SizedBox(width: 10),
              _buildUrgensiChip(
                  "Kritis", "kritis", Icons.report_rounded, Colors.red),
            ],
          ),
          const SizedBox(height: 12),
          Container(
    ...
    return Container(
      ...
      child: ElevatedButton(
        ...
        style: ElevatedButton.styleFrom(
            ...
            shape: RoundedRectangleBorder(
                borderRadius: BorderRadius.circular(18))),
        child: const Text("AJUKAN LEMBUR",
            style: TextStyle(
                fontSize: 16,
  ...
  Future<void> _submit() async {
    ...
    if (!_allDataComplete()) {
      ...
      if (selectedMiras.isEmpty)
        missingField = "Pilih mitra terlebih dahulu";
      else if (tanggalLembur == null)
        missingField = "Pilih tanggal lembur";
      else if (jamMulai == null || jamSelesai == null)
        missingField = "Pilih waktu lembur";
      else if (_selectedLocation == null)
        missingField = "Pilih lokasi lembur";
      else if (alasanController.text.isEmpty)
        missingField = "Isi alasan lembur";
      else if (alasanController.text.length < 20)
        missingField = "Alasan minimal 20 karakter";
    ...
    for (final mitra in selectedMiras) {
      final result = await _mitraLimitService.checkMitraLimit(
          mitraId: mitra['id'], tanggal: tanggalLembur!, tambahanJam: totalJam);
  ...
  Future<void> _saveOvertimeData(bool hasLimitExceeded, String groupId) async {
    ...
    batch.set(pengajuanRef, {
      ...
      'detail_mitra': selectedMiras
          .map((m) =>
              {'id': m['id'], 'nama': m['nama_lengkap'], 'fungsi': m['fungsi']})
          .toList(),
      'urgensi': urgensi,
    ...
    final baseMitraData = {
      ...
      'estimasi_biaya_per_mitra':
          selectedMiras.isNotEmpty ? totalBiaya / selectedMiras.length : 0,
      'rate_snapshot': _overtimeRates,
      'status': 'pending',
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Text("${selectedMiras.length} mitra telah didaftarkan",
  ...
  Future<bool> _showLimitWarningDialog(
      Map<String, dynamic> limitResults) async {
          ...
          builder: (context) => AlertDialog(
            ...
            content: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 12),
                  const Text(
                      "Pengajuan akan ditandai sebagai KRITIS dan memerlukan persetujuan khusus.",
          ...
          builder: (context) => AlertDialog(
            ...
            content: Column(
                ...
                children: [
                  ...
                  _buildConfirmRow(
                      "📅 Tanggal", _formatTanggal(tanggalLembur!)),
                  _buildConfirmRow(
                      "📆 Jenis", isWeekend ? "Hari Libur" : "Hari Kerja"),
                  _buildConfirmRow("⏰ Waktu",
                      "${_formatTime(jamMulai!)} - ${_formatTime(jamSelesai!)}"),
                  _buildConfirmRow(
                      "⏱️ Durasi", "${totalJam.toStringAsFixed(1)} jam"),
                  _buildConfirmRow("💰 Total Biaya",
...
class _MapPickerDialogState extends State<_MapPickerDialog> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("Pilih Lokasi Proyek",
            style: TextStyle(fontWeight: FontWeight.bold)),
